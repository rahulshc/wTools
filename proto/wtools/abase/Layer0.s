//#! /usr/bin/env node
(function _wToolsLayer0_s_(){

'use strict';

if( typeof module !== 'undefined' )
{

  require( './l0/l1/Predefined.s' );

  require( './l0/l2/Setup.s' );

  require( './l0/l3/iArray.s' );
  require( './l0/l3/iBool.s' );
  require( './l0/l3/iBuffer.s' );
  require( './l0/l3/iContainer.s' );
  require( './l0/l3/iCt.s' );
  require( './l0/l3/iEntity.s' );
  require( './l0/l3/iErr.s' );
  require( './l0/l3/iFunctional.s' );
  require( './l0/l3/iInterval.s' );
  require( './l0/l3/iIntrospector.s' );
  require( './l0/l3/iLong.s' );
  require( './l0/l3/iMap.s' );
  require( './l0/l3/iModule.s' );
  require( './l0/l3/iNumber.s' );
  require( './l0/l3/iProcess.s' );
  require( './l0/l3/iRegexp.s' );
  require( './l0/l3/iRoutine.s' );
  require( './l0/l3/iSet.s' );
  require( './l0/l3/iStr.s' );
  require( './l0/l3/iTime.s' );
  require( './l0/l3/iType.s' );
  require( './l0/l3/iUnroll.s' );

  require( './l0/l5/fArray.s' );
  require( './l0/l5/fBool.s' );
  require( './l0/l5/fBuffer.s' );
  require( './l0/l5/fContainer.s' );
  require( './l0/l5/fCt.s' );
  require( './l0/l5/fEntity.s' );
  require( './l0/l5/fErr.s' );
  require( './l0/l5/fEvent.s' );
  require( './l0/l5/fFunctional.s' );
  require( './l0/l5/fInterval.s' );
  require( './l0/l5/fIntervalc.s' );
  require( './l0/l5/fIntervall.s' );
  require( './l0/l5/fIntervalo.s' );
  require( './l0/l5/fIntervalOld.s' );
  require( './l0/l5/fIntrospector.s' );
  require( './l0/l5/fLong.s' );
  require( './l0/l5/fMap.s' );
  require( './l0/l5/fModule.s' );
  require( './l0/l5/fNumber.s' );
  require( './l0/l5/fPath.s' );
  require( './l0/l5/fProcess.s' );
  require( './l0/l5/fRegexp.s' );
  require( './l0/l5/fRoutine.s' );
  require( './l0/l5/fSorted.s' );
  require( './l0/l5/fSet.s' );
  require( './l0/l5/fStr.s' );
  require( './l0/l5/fTime.s' );
  require( './l0/l5/fUnroll.s' );

  require( './l0/l6/ContainerAdapter.s' );
  require( './l0/l6/LongDescriptor.s' );

  require( './l0/l7/Compose.s' );
  require( './l0/l7/ContainerAdapterArray.s' );
  require( './l0/l7/ContainerAdapterSet.s' );
  require( './l0/l7/Field.s' );
  require( './l0/l7/LongContext.s' );

  require( './l0/l8/gArray.s' );
  require( './l0/l8/gBool.s' );
  require( './l0/l8/gBuffer.s' );
  require( './l0/l8/gContainer.s' );
  require( './l0/l8/gCt.s' );
  require( './l0/l8/gEntity.s' );
  require( './l0/l8/gErr.s' );
  require( './l0/l8/gFunctional.s' );
  require( './l0/l8/gInterval.s' );
  require( './l0/l8/gIntrospector.s' );
  require( './l0/l8/gLong.s' );
  require( './l0/l8/gMap.s' );
  require( './l0/l8/gModule.s' );
  require( './l0/l8/gNumber.s' );
  require( './l0/l8/gPair.s' );
  require( './l0/l8/gProcess.s' );
  require( './l0/l8/gRegexp.s' );
  require( './l0/l8/gRoutine.s' );
  require( './l0/l8/gSet.s' );
  require( './l0/l8/gStr.s' );
  require( './l0/l8/gTime.s' );

  require( './l0/l9/Setup.s' );

}

let _global = _global_;
let _ = _global_.wTools;
let Self = _global_.wTools;

// --
// export
// --

if( typeof module !== 'undefined' )
module[ 'exports' ] = Self;

})();
