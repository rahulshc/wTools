...
{
  test.case = 'two arguments, src - number, in crange';
  var got = _.crange.assertInRange( 3, [ 1, 5 ] );
  var expected = true;
  test.identical( got, expected );
  ...
  test.case = 'more then two arguments, src - number, in crange';
  var got = _.crange.assertInRange( 3, [ 1, 5 ], 'extra', [ 'next' ] );
  var expected = true;
  test.identical( got, expected );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.crange.assertInRange() );

  test.case = 'not enough arguments';
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 2 ) );

  test.case = 'src out of crange';
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 1, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 6, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 7, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 1, [ 2, 5 ], [] ) );
  test.shouldThrowErrorSync( () => _.crange.assertInRange( 6, [ 2, 5 ], {} ) );
...
function crangeClamp( test )
{
  ...
  var dst = [ 2, 2 ];
  var clamp = [ 2, 2 ];
  var got = _.crange.clamp( dst, clamp );
  test.identical( got, [ 2, 2 ] );
  test.is( got === dst );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.crange.clamp() );

  test.case = 'not enough arguments';
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 1, 2 ] ) );

  test.case = 'extra arguments';
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 1, 2 ], [ 3, 4 ], [ 5, 6 ] ) );

  test.case = 'dstRange is not a crange';
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 1, 2, 3 ], [ 3, 4 ] ) );
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 1, 'wrong' ], [ 3, 4 ] ) );
  test.shouldThrowErrorSync( () => _.crange.clamp( [ undefined, 1 ], [ 3, 4 ] ) );

  test.case = 'clampRange is not a crange';
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 3, 4 ], [ 1, 2, 3 ] ) );
  test.shouldThrowErrorSync( () => _.crange.clamp( [ 3, 4 ], [ 1, 'wrong' ] ) );
...
{
  test.case = 'lrange - number';
  var got = _.lrange.fromSingle( 1 );
  test.identical( got, [ 1, 1 ] );

  /* */

  test.case = 'lrange.length === 1';
  var src = [ 1 ];
  var got = _.lrange.fromSingle( src );
  test.identical( got, [ 1, 1 ] );
  test.is( got !== src );

  test.case = 'lrange[ 0 ] - undefined';
  var src = [ undefined, 1 ];
  var got = _.lrange.fromSingle( src );
  test.identical( got, [ 0, 1 ] );
  test.is( got !== src );

  test.case = 'lrange[ 1 ] - undefined';
  var src = [ 1, undefined ];
  var got = _.lrange.fromSingle( src );
  test.identical( got, [ 1, 1 ] );
  test.is( got !== src );

  test.case = 'lrange[ 0 ] - undefined, lrange[ 1 ] - undefined';
  var src = [ undefined, undefined ];
  var got = _.lrange.fromSingle( src );
  test.identical( got, [ 0, 1 ] );
  test.is( got !== src );

  test.case = 'lrange[ 0 ] - number, lrange[ 1 ] - number';
  var src = [ 1, 5 ];
  var got = _.lrange.fromSingle( src );
  test.identical( got, [ 1, 5 ] );
  test.is( got === src );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.lrange.fromSingle() );

  test.case = 'extra arguments';
  test.shouldThrowErrorSync( () => _.lrange.fromSingle( [ 1, 2 ], 1 ) );

  test.case = 'wrong type of lrange';
  test.shouldThrowErrorSync( () => _.lrange.fromSingle( { 0 : 1, 1 : 2 } ) );

  test.case = 'wrong lrange length';
  test.shouldThrowErrorSync( () => _.lrange.fromSingle( [ 0, 1, 2 ] ) );

  test.case = 'wrong elements in lrange';
  test.shouldThrowErrorSync( () => _.lrange.fromSingle( [ null, 2 ] ) );
...
{
  test.open( 'lrange - array' );

  test.case = 'without options';
  var got = _.lrange.lastGet( [ 2, 3 ] );
  test.identical( got, 4 );

  test.case = 'options - undefined';
  var options = undefined;
  var got = _.lrange.lastGet( [ 2, 3 ], options );
  test.identical( got, 4 );
  test.identical( options, undefined );
  ...
  test.close( 'lrange - array' );

  /* - */

  test.open( 'lrange - unroll' );

  test.case = 'without options';
  var got = _.lrange.lastGet( _.unrollMake( [ 2, 3 ] ) );
  test.identical( got, 4 );

  test.case = 'options - undefined';
  var options = undefined;
  var got = _.lrange.lastGet( _.unrollMake( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, undefined );
  ...
  test.close( 'lrange - unroll' );

  /* - */

  test.open( 'lrange - argumentsArray' );

  test.case = 'without options';
  var got = _.lrange.lastGet( _.argumentsArrayMake( [ 2, 3 ] ) );
  test.identical( got, 4 );

  test.case = 'options - undefined';
  var options = undefined;
  var got = _.lrange.lastGet( _.argumentsArrayMake( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, undefined );
  ...
  test.close( 'lrange - argumentsArray' );

  /* - */

  test.open( 'lrange - BufferTyped' );

  test.case = 'without options';
  var got = _.lrange.lastGet( new U8x( [ 2, 3 ] ) );
  test.identical( got, 4 );

  test.case = 'options - undefined';
  var options = undefined;
  var got = _.lrange.lastGet( new I16x( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, undefined );

  test.case = 'options - empty map';
  var options = {};
  var got = _.lrange.lastGet( new F32x( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, { increment : 1 } );

  test.case = 'options - map with own field increment';
  var options = { increment : 2 };
  var got = _.lrange.lastGet( new F64x( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, { increment : 2 } );

  test.case = 'options - map with own field increment - undefined';
  var options = { increment : undefined };
  var got = _.lrange.lastGet( new I8x( [ 2, 3 ] ), options );
  test.identical( got, 4 );
  test.identical( options, { increment : 1 } );

  test.close( 'lrange - BufferTyped' );

  /* - */

  test.open( 'lrange - map' );

  test.case = 'without options, last - undefined';
  var got = _.lrange.lastGet( { first : undefined, last : undefined } );
  test.identical( got, undefined );

  test.case = 'without options, last - not exists';
  var got = _.lrange.lastGet( { first : undefined } );
  test.identical( got, undefined );

  test.case = 'without options, last - number';
  var got = _.lrange.lastGet( { first : 1, last : 2 } );
  test.identical( got, 2 );
  ...
  test.case = 'options - undefined, last - undefined';
  var options = undefined;
  var got = _.lrange.lastGet( { first : undefined, last : undefined }, options );
  test.identical( got, undefined );
  test.identical( options, undefined );

  test.case = 'options - undefined, last - not exists';
  var options = undefined;
  var got = _.lrange.lastGet( { first : undefined }, options );
  test.identical( got, undefined );
  test.identical( options, undefined );

  test.case = 'options - undefined, last - number';
  var options = undefined;
  var got = _.lrange.lastGet( { first : 1, last : 2 }, options );
  test.identical( got, 2 );
  test.identical( options, undefined );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.lrange.lastGet() );

  test.case = 'extra arguments';
  test.shouldThrowErrorSync( () => _.lrange.lastGet( [ 2, 3 ], {}, 'extra' ) );

  test.case = 'lrange is a long, but is not a lrange';
  test.shouldThrowErrorSync( () => _.lrange.lastGet( [ 1, 2, 3 ] ) );
  test.shouldThrowErrorSync( () => _.lrange.lastGet( [ 1, 'wrong' ] ) );
  test.shouldThrowErrorSync( () => _.lrange.lastGet( [ undefined, 1 ] ) );

  test.case = 'wrong type of lrange';
  test.shouldThrowErrorSync( () => _.lrange.lastGet( new Set( [ 1, 2 ] ) ) );
  test.shouldThrowErrorSync( () => _.lrange.lastGet( new Map( [ [ 1, 2 ] ] ) ) );
}
...
{
  test.case = 'two arguments, src - number, in orange';
  var got = _.orange.assertInRange( 3, [ 1, 5 ] );
  var expected = true;
  test.identical( got, expected );
  ...
  test.case = 'more then two arguments, src - number, in orange';
  var got = _.orange.assertInRange( 3, [ 1, 5 ], 'extra', [ 'next' ] );
  var expected = true;
  test.identical( got, expected );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.orange.assertInRange() );

  test.case = 'not enough arguments';
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 2 ) );

  test.case = 'src out of orange';
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 1, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 5, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 7, [ 2, 5 ] ) );
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 1, [ 2, 5 ], [] ) );
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 5, [ 2, 5 ], {} ) );
  test.shouldThrowErrorSync( () => _.orange.assertInRange( 7, [ 2, 5 ], undefined ) );
}
...
function orangeClamp( test )
{
  ...
  var dst = [ 2, 2 ];
  var clamp = [ 2, 2 ];
  var got = _.orange.clamp( dst, clamp );
  test.identical( got, [ 2, 2 ] );
  test.is( got === dst );
  ...
  test.case = 'without arguments';
  test.shouldThrowErrorSync( () => _.orange.clamp() );

  test.case = 'not enough arguments';
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 1, 2 ] ) );

  test.case = 'extra arguments';
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 1, 2 ], [ 3, 4 ], [ 5, 6 ] ) );

  test.case = 'dstRange is not a orange';
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 1, 2, 3 ], [ 3, 4 ] ) );
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 1, 'wrong' ], [ 3, 4 ] ) );
  test.shouldThrowErrorSync( () => _.orange.clamp( [ undefined, 1 ], [ 3, 4 ] ) );

  test.case = 'clampRange is not a orange';
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 3, 4 ], [ 1, 2, 3 ] ) );
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 3, 4 ], [ 1, 'wrong' ] ) );
  test.shouldThrowErrorSync( () => _.orange.clamp( [ 3, 4 ], [ undefined, 1 ] ) );
}
